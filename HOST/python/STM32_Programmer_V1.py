...
COMMAND_BL_EN_R_W_PROTECT_LEN                       = 9
...
def calc_file_len():
    size = os.path.getsize("002USER_Application.bin")
    return size

def open_the_file():
    global bin_file
    bin_file = open('002USER_Application.bin','rb')
    #read = bin_file.read()
...
def protection_type(s_status,n):
    if( s_status[1] & (1 << 7) ):
        #PCROP is active
        if(n < 8):
            if(s_status[0] & (1 << n) ):
...
def process_COMMAND_BL_READ_SECTOR_STATUS(length):
    ...
    print("\n   Sector Status : ", "MSB = ", hex(s_status[1]), " | LSB = ", hex(s_status[0]))
    print("\n  ====================================")
    print("\n  Sector                               \tProtection") 
    print("\n  ====================================")
    if(s_status[1] & (1 << 7)):
    ...
    for x in range(12):
        print("\n   Sector{0}                               {1}".format(x,protection_type(s_status,x) ) )
